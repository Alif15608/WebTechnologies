...
<body>
	...
	$name = $email = $mobilenumber = $address = $username = $password = $confirmpassword = $gender = $dob = "";
	$nameErr = $emailErr = $mobilenumberErr = $addressErr = $usernameErr = $passwordErr = $confirmpasswordErr = $genderErr = $dobErr = $message = "";
	$isValid = true;
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if($isValid){
			...
			if(!file_exists('../Model/manager.json')){
				@file_put_contents('../Model/manager.json', '');
			}

			$data = json_decode(file_get_contents('../Model/manager.json'), true);
			...
			if(empty($userExist)){
				$data[] = $set;
				file_put_contents('../Model/manager.json', json_encode($data));
