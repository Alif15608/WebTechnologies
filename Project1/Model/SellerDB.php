<?php 
    ...
    function edit($id, $name, $gender, $dob, $phone, $email) {
        $ezl = connect();
        $sql = "UPDATE seller SET Name='$name', Email='$email', Gender='$gender', DateOfBirth='$dob', Contact='$phone' WHERE ID=$id";
        $qry = $ezl->query($sql);

        header("location: /Project1/View/viewprofile.php");
    }

    function password($id, $newpassword) {
        $ezl = connect();
        $sql = "UPDATE seller SET Password='$newpassword' WHERE ID='$id'";
        $qry = $ezl->query($sql);

        setcookie('msg', '<b>✅Password Changed</b><br>', time() + 1, '/');
        header("location: /Project1/View/changepassword.php");
    }
    ...
    function user($user, $id) {
        ...
        $sql = "UPDATE seller SET Username='$user' WHERE ID='$id'";
        $qry = $ezl->query($sql);
